...
async def populate_admin_cache():
    ...
    logger.info("Admin cache populated: %s", _admin_id_cache)
...
def register_admin_handlers(bot: AsyncTeleBot):
    ...
    async def guard(uid: int, event) -> bool:
        # Check DB first, then fall back to ADMIN_IDS env var as safety net
        is_adm = await db.is_admin(uid) or (uid in ADMIN_IDS)
        logger.info("guard() uid=%s is_admin=%s", uid, is_adm)
        if not is_adm:
            ...
            return False
        # Ensure admin is in DB (auto-add if missing)
