...
from telebot.types import Message, CallbackQuery, BufferedInputFile
...
def register_admin_handlers(bot: AsyncTeleBot):
    ...
    # ── Admin text/photo FSM handler — registered at group=1 (fires first) ────
    # This handler intercepts messages ONLY when admin is in an admin FSM state.
    # It returns early for all other states so the user handler catches them.
    @bot.message_handler(
        func=lambda m: True,
    ...
    async def handle_admin_fsm(msg: Message):
        ...
        if not await db.is_admin(uid):
            return  # Not an admin — let user_handler catch it

        state, data = await db.get_session(uid)
        if state not in States.ALL_ADMIN_INPUT:
            return  # Not in admin FSM — let user_handler catch it
...
async def _do_export(bot: AsyncTeleBot, chat_id: int):
    ...
    # BufferedInputFile is the correct class in pyTelegramBotAPI 4.x
