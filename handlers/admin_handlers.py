...
def register_admin_handlers(bot: AsyncTeleBot):
    ...
    # func=lambda ONLY matches messages from known admin IDs.
    # Non-admin messages are NEVER consumed here — they fall through to user handler.
    # State check inside body ensures we only act on admin FSM states.
    @bot.message_handler(
        func=lambda m: m.from_user is not None and m.from_user.id in _admin_id_cache,
    ...
    async def handle_admin_fsm(msg: Message):
        ...
        if state not in States.ALL_ADMIN_INPUT:
            return  # Admin but not in FSM state — fall through to user handler
